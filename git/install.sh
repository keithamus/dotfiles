...
if [ "$(which apt 2>/dev/null)" ]; then
  # Git-Delta
  wget -q https://github.com/dandavison/delta/releases/download/0.1.1/git-delta_0.1.1_amd64.deb
...
GPG_ID=$(gpg --list-secret-keys --keyid-format LONG "$GITEMAIL" 2>/dev/null | head -1 | awk '{print $2}' | awk -F '/' '{print $2}')
if [ "$GPG_ID" ]; then
